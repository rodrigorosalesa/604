...
$\widehat{y}_{daily.cases} = - 317.9720 - 0.0000004255 x_{population} + 69.5871 x_{aged.65.older} - 0.0702 x_{gdp.per.capita} + 0.000000005053 x_{population} x_{gdp.per.capita} + 0.0100 x_{aged.65.older} x_{gdp.per.capita}$\
Our model has an Adjusted R Squared of 0.6551. Our RSME is 6208.4082.
...
$\widehat{y}_{daily.deaths} = 0.8449 - 0.000004976 x_{population} + 10.9375 x_{human.development.index} + 0.000008337 x_{population} x_{human.development.index}$\
More important our Adjusted R Squared is now 0.5141. Our RMSE = 87.5619.
...
$\widehat{y}_{new.daily.cases} = - 317.9720 - 0.0000004255 x_{population} + 69.5871 x_{aged.65.older} - 0.0702 x_{gdp.per.capita} + 0.000000005053 x_{population} x_{gdp.per.capita} + 0.0100 x_{aged.65.older} x_{gdp.per.capita}$\
Adjusted R Squared = 0.6551. RSME = 6,208.4082.
...
$\widehat{y}_{daily.deaths} = 0.8449 - 0.000004976 x_{population} + 10.9375 x_{human.development.index} + 0.000008337 x_{population} x_{human.development.index}$\
Adjusted R Squared = 0.5141. RMSE = 87.5619.
...
The coefficient estimates for the focal variables in the final model above imply the following:
  ...
  - *reproduction_rate*: for every increase of 1.0 in the daily estimate of the reproduction rate of COVID-19 (that is, the average number of people who will be infected by someone who is infected with the virus), the number of predicted new daily cases (averaged over 7 days) changes by -177.4731 - (0.0028 * new_tests_smoothed)  - (0.0114 * new_vaccinations_smoothed) + 0.0505 (new_people_vaccinated_smoothed) + (58.5161 * stringency_index) - (1.4194 *  hosp_patients).  
...
After performing the necessary statistical analyses for our linear models, the team has uncovered for Research Question 1 that the most important population-metrics for daily COVID deaths and cases include: population, elderly population, GDP per capita, and HDI. For Research Question 2, the team has concluded that vaccinations, government regulations, and COVID-19 tests are significant predictors of daily COVID deaths and cases.

For Research Question 1, the modeling exercise when assessing the average new daily COVID-19 cases did not pass the homoscedasticity and normality assumptions, however did pass the multicollinearity, linearity, and independent error assumptions. The model yielded an Adjusted R-Squared value of 65.55%. With these facts coupled together, the model for Research Question 1 is not very accurate and trustworthy to draw conclusions from.

When assessing the second part of Research Question 1, average daily deaths from COVID-19, the only assumption the model did not pass is the normality assumption, albeit passed homoscedasticity, multicollinearity, independent errors and linearity assumptions. The Adjusted R-Squared value yielded was 51.41%. As the model fails to pass the normality assumption and the Adjusted R-Squared value is low, the model is deemed as untrustworthy and therefore not completely reliable. 

For Research Question 2, the team sought out to create the best model to predict daily COVID-19 cases and deaths in a country. For the daily new COVID-19 cases, the model failed the normality assumption, but passed the homoscedasticity, linearity, independent errors, and multicollinearity assumptions, yielding an Adjusted R-Squared value of 51.41%, which is not very impressive. For this reason, we deem this model as untrustworthy to use it for accurate predictions.

Additionally, for the rest of Research Question 2, the factor assessed is the daily new COVID-19 deaths.  This model did not pass the homoscedasticity, normality assumptions, while the linearity and independent error assumption passes. This model had the highest Adjusted R-Squared value for any of the team’s models, at about 91.97%. However, as the Adjusted R-Squared is not the only thing used to deem a model’s success, the team concludes that this model is again untrustworthy to be used to predict data accurate to reality as it fails two of the assumptions.
...
Next, possible future datasets that could be used to better assess what factors were at play when it comes to quantifying the severity of the COVID-19 pandemic include:

- Stock Index Data: Are countries with a higher performing stock index better equipped for dealing with a global pandemic? Does the performance of the stock market in a specific country indicate any correlations with how COVID-19 was handled in that country?
...
Ultimately, through the creation of various multiple linear regression models to uncover data-driven answers to our 2 Research Questions, the team was able to yield results. Despite the models being unreliable to base conclusions that are robust and indisputable, the team has investigated the research questions to the best of their ability. It has been discovered that the population, elderly population, GDP per capita, and HDI are the most strongly related to the prevalence and severity of COVID-19. Moreover, the team has also concluded that all three predictors of vaccinations, government regulations, and COVID-19 testing are heavily related predictors of the severity and prevalence of COVID-19.  

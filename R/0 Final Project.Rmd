...
$H_0: \beta_i = 0$\
$H_a: \beta_i \neq 0$\
$(i = \text{(population, aged.65.older, gdp.per.capita)^2})$
...
ggpairs(covid_higher_order, lower = list(continuous="smooth_loess", combo="facethist", discrete="facetbar", na="na")) + ggtitle('Figure 1: Correlation for terms present in Model Average Daily Cases')
```
...
```
```{r}
plot(model_cases_interactions_2, which=5) + title('Figure 3: Hook´s Distance Plot for Average Daily Covid-19 Cases
                                                  ')
```
...
```{r}
ggplot(model_cases_interactions_2, aes(x=.fitted, y=.resid)) + geom_point() + geom_smooth()+ geom_hline(yintercept = 0) + ggtitle('Figure 4: Rresiduals vs. Fitted Values for Average Daily Covid-19 Cases')
```
...
#### Final Model
As we shown, our Final Model did not pass the following assumptions: Homoscedasticity, Normality, Independant Errors and Linearity; only passing the Multicolinearity one. When using Box Cox transformation we got an error saying that *response variable must be positive*, looking online we could have used the Yeo-Johnson transformation, however it is outside of the scope of the course so we did not do it. 

The final model to predict the average new daily Covid-19 cases for a specific country using is:\
$\widehat{y}_{new.daily.cases} = - 317.9720 - 0.0000004255 x_{population} + 69.5871 x_{aged.65.older} - 0.0702 x_{gdp.per.capita} + 0.000000005053 x_{population} x_{gdp.per.capita} + 0.0100 x_{aged.65.older} x_{gdp.per.capita}$\
Our model has an Adjusted R Squared of 0.6551.

We decided to declare the previously constructed model as the final model for average daily new cases fo question 1, despite its violation of the assumptions. It is important to note, therefore, that the results of the model are not necessarily trustworthy.
```{r}
...
The model obtained after performing the *Step Wise Regression Procedure* is:\
$\widehat{y}_{new.deaths} = - 125.4150 + 0.0000005815 x_{population} - 198.6108 x_{human.development.index}$
...
$H_0: \beta_i = 0$\
$H_a: \beta_i \neq 0$\
$(i = \text{(population, human_development_index)^2})$
...
$\widehat{y}_{new.deaths} = 0.8449 - 0.000004976 x_{population} + 10.9375 x_{human.development.index} + 0.000008337 x_{population} x_{human.development.index}$
...
ggpairs(covid_higher_order, lower = list(continuous="smooth_loess", combo="facethist", discrete="facetbar", na="na")) + ggtitle('Figure 5: Correlation for terms present in Model Average Daily Deaths')
```
...
Plotting a Q-Q plot:
```{r}
ggplot(model_deaths_interactions, aes(sample=model_deaths_interactions$residuals)) + stat_qq(color='blue') + stat_qq_line(color='red') + ggtitle('Figure 6: Q-Q plot for Average Daily Covid-19 Deaths')
```
...
Testing for linearity using a residuals vs predicted $\hat{Y}$ plot:
```{r}
ggplot(model_deaths_interactions, aes(x=.fitted, y=.resid)) + geom_point() + geom_smooth()+ geom_hline(yintercept = 0) + ggtitle('Figure 7: Residuals vs Rredicted Values for Average Daily Covid-19 Deaths')
```
...
```{r}
plot(model_deaths_revised,which=5) + title('Figure 8: Residuals vs Leverage Plot for Average Daily Covid-19 Deaths
                                           ')
```
...
Plotting residuals vs. fitted values to see if the error terms look independent from each other:
```{r}
ggplot(model_deaths_interactions, aes(x=.fitted, y=.resid)) + geom_point() + geom_smooth()+ geom_hline(yintercept = 0) + ggtitle('Figure 9: Residuals vs Fitted Values for Average Daily Covid-19 Deaths')
```
...
We decided to declare the previously constructed model as the final model for average daily new deaths fo question 1, despite its violation of the assumptions. It is important to note, therefore, that the results of the model are not necessarily trustworthy.
```{r}
...
We get no rows where new deaths is less than 0, so we can continue with the Box-Cox transformation.

```{r}
bc=boxcox(gq2_deaths_intermod_reduced,lambda=seq(-1,1))
bestlambda=bc$x[which(bc$y==max(bc$y))]
bestlambda
```
When attempting to conduct a Box Cox transformation, we received an error saying "response variable must be positive", despite having confirmed previously that the response variable is never less than 0 (see above). After researching this issue, we found that we could have used the Yeo-Johnson transformation; however, this is beyond the scope of this course, so we decided not to do it.  

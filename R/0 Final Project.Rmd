...
#tenth_percentile_cases <- quantile(covid_raw2$new_cases_smoothed_per_million, probs = 0.05, na.rm = TRUE)
#bad_country_cases <- covid_raw2[covid_raw2$new_cases_smoothed_per_million < tenth_percentile_cases,]
...
#tenth_percentile_deaths <- quantile(covid_raw2$new_deaths_smoothed_per_million, probs = 0.05, na.rm = TRUE)
#bad_country_deaths <- covid_raw2[covid_raw2$new_deaths_smoothed_per_million < tenth_percentile_deaths,]
...
#good_countries <-   covid_raw2[!covid_raw2$location %in% bad_country_list, ]
...
# Getting countries with bottom 5% of new_cases_smoothed_per_million
tenth_percentile_cases <- quantile(covid_agg$new_cases_smoothed_per_million, probs = 0.05, na.rm = TRUE)
bad_country_cases <- covid_agg[covid_agg$new_cases_smoothed_per_million < tenth_percentile_cases,]
bad_country_list_cases <- unique(bad_country_cases$location)

# Getting countries with bottom 5% of new_deaths_smoothed_per_million
tenth_percentile_deaths <- quantile(covid_agg$new_deaths_smoothed_per_million, probs = 0.05, na.rm = TRUE)
bad_country_deaths <- covid_agg[covid_agg$new_deaths_smoothed_per_million < tenth_percentile_deaths,]
bad_country_list_deaths <- unique(bad_country_deaths$location)
...
$\widehat{y}_{new.cases} = 130881.628  - 0.24490991 x_{new_tests} + 0.24490991 x_{population} +4.6709x_{hosp.patients} + 1397.858 x_{aged.65.older}- 54.705     x_{cardiovasc.death.rate}- 1642.655 x_{life.expectancy} -371.30791551x_{extreme.poverty}+ 0.20325820 x_{gdp.per.capita}- 468.88355355x_{meidan.age}+ 218.7925843 x_{smokers}$

Adjusted R Squared of our model is: 0.9391 , meaning that the proportion of the total variation that is explained by the model is 93.91%.

```{r}
model_cases_revised = lm(new_cases ~ new_tests + population + hosp_patients + aged_65_older + cardiovasc_death_rate +life_expectancy+aged_65_older+extreme_poverty+gdp_per_capita +median_age +smokers, data=covid_agg)
...
Using interactions terms can be a powerful way to increase the efficiency of our model as sometimes the effect of an independent variable on a dependent variable, in our case new Covid-19 cases, is not constant over all of the values of the other independent variables. 
...
```{r}
model_cases_interactions = lm(new_cases ~ (new_tests + population + hosp_patients + aged_65_older + cardiovasc_death_rate +life_expectancy+aged_65_older+extreme_poverty+gdp_per_capita +median_age +smokers)^2, data=covid_agg)
summary(model_cases_interactions)
```

```{r}
model_cases_interactions_2 <- lm(new_cases ~ new_tests + population + hosp_patients + aged_65_older + cardiovasc_death_rate +life_expectancy+aged_65_older+extreme_poverty+gdp_per_capita +median_age +smokers + new_tests:hosp_patients+new_tests:life_expectancy+new_tests:gdp_per_capita+new_tests:median_age+population:hosp_patients+population:aged_65_older+population:cardiovasc_death_rate+population:life_expectancy+population:median_age+population:smokers+hosp_patients:life_expectancy+hosp_patients:gdp_per_capita+hosp_patients:smokers, data=covid_agg)
summary(model_cases_interactions_2)
```

Our new model with interaction terms has an adjusted R-squared value of 0.9991 which is 0.06 larger than our model without interaction terms. This means it explains 99.91% of variance in new_cases. The new model is: 

$\widehat{y}_{new.cases} = -979.1904  - 1.6289  x_{new_tests} + 0.000313 x_{population} + 51.7419x_{hosp.patients} + 19.5443 x_{aged.65.older}+ 2.81409        x_{cardiovasc.death.rate}+ 11.1993 x_{life.expectancy} -22.2164 x_{extreme.poverty}+ 0.009387 x_{gdp.per.capita}- 17.2106 x_{meidan.age} - 13.3943 x_{smokers} + 0.00007233 x_{new_tests:hosp_patients}+ 0.0247 x_{new_tests:life_expectancy}-0.00000351 x_{new_tests:gdp.per.capita}-0.006533 x_{new_tests:meidan.age}- 0.000000281 x_{population:hosp_patients}+0.00001372 x_{population:aged_65_older}- 0.000000145 x_{population:cardiovasc.death.rate}- 0.000008162 x_{population:life.expectancy}+0.00001176 x_{population:median.age}-0.00000141 x_{population:smokers}- 0.6344 x_{hosp.patients:life.expectancy}+0.000142 x_{hosp.patients:gdp.per.capita}-0.1519x_{hosp.patients:smokers}$

Because the adjusted R-squared of our interaction model is already so high, we do not think it is worth checking for higher order terms due to diminishing returns in adjusted R-squared increases vs model complexity.
...
From the output of our test, we get a p-value of 0.000004655 which is less than 0.05. This means we can  reject our null hypothesis that our sample data is significantly normally distributed and conclude with a significance level of 0.05 that our sample data is not normally distributed. This means that there is a problem with the normality assumption.
...
From the output of our test, we get a p-value of 0.1541 which is greater than 0.05. This means we fail to reject the null hypothesis that there is homoscedasticity and conclude with a significance level of 0.05 that our model is not homoscedastic. This means that there is no problem with the homoscedasticity assumption. 
...
```{r}
lev=hatvalues(model_cases_interactions_2)
p = length(coef(model_cases_interactions_2))
n = nrow(covid_agg)
...
From our output, we have no influential outlier in our data at rows 12, 72, 77, 90, 95, 96, 102, and 104. Removing these rows:

```{r}
covid_agg2 <- covid_agg[-c(12,72,77,90,95,96,102,104),]
...
```{r}
model_cases_interactions_3 <- lm(new_cases ~ new_tests + population + hosp_patients + aged_65_older + cardiovasc_death_rate +life_expectancy+aged_65_older+extreme_poverty+gdp_per_capita +median_age +smokers + new_tests:hosp_patients+new_tests:life_expectancy+new_tests:gdp_per_capita+new_tests:median_age+population:hosp_patients+population:aged_65_older+population:cardiovasc_death_rate+population:life_expectancy+population:median_age+population:smokers+hosp_patients:life_expectancy+hosp_patients:gdp_per_capita+hosp_patients:smokers, data=covid_agg2)
summary(model_cases_interactions_3)
```

Our new model with interaction terms has an adjusted R-squared value of 0.9993 which is 0.0002 larger than our model without interaction terms. This means it explains 99.93% of variance in new_cases. The new model is: 

$\widehat{y}_{new.cases} = -1916.38435235562  - 1.581 x_{new_tests} + 0.000314  x_{population} + 42.855 {hosp.patients} - 30.7078 x_{aged.65.older}+ 3.2676           x_{cardiovasc.death.rate}+ 23.995 x_{life.expectancy} -22.27244 x_{extreme.poverty}+ 0.00191 x_{gdp.per.capita}- 4.00416 x_{meidan.age} - 18.437 x_{smokers} + 0.0000746 x_{new_tests:hosp_patients}+ 0.0241   x_{new_tests:life_expectancy}-0.0000018 x_{new_tests:gdp.per.capita}-0.007222  x_{new_tests:meidan.age}- 0.000000303 x_{population:hosp_patients}+0.0000152 x_{population:aged_65_older}- 0.00000014 x_{population:cardiovasc.death.rate}- 0.0000081 x_{population:life.expectancy}+0.0000113 x_{population:median.age}-0.0000014 x_{population:smokers}- 0.5114    x_{hosp.patients:life.expectancy}+0.000089 x_{hosp.patients:gdp.per.capita}-0.125 x_{hosp.patients:smokers}$
